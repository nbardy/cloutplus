...
vec4 sdSerp(in vec3 p,in float h,float D)
{
    vec3 top=vec3(0.);
    //base if above the point
    vec3 base=top+vec3(0.,h,0.);
    ...
    // NOTE: This has to be fixed(wish it was D)
...
// sierpinski inverted
vec4 sdSerpInv(in vec3 p,in float h,float D)
{
    vec3 top=vec3(0.);
    //base if above the point
    vec3 base=top+vec3(0.,h,0.);
...
// Returns the small dist vec with attachment text value(2,1)
// mat3 format is [distance, textureid, empty], [normxyz], [norm]
mat3 dmin(vec4 v1,vec4 v2){
    mat3 res;
    if(v2.x>v1.x){
        res[0]=vec3(v1.x,2.,0.);
        res[1]=vec3(v1.yzw);
...
mat3 map(in vec3 pos,vec3 ro)
{
    ...
    if(t<stillT){
        ...
        vec4 r1=sdSerp(p+vec3(0.,starty,0.),H,1.);
    ...
    vec4 r1=sdSerp(p+rs1,H,Dmax);
    vec4 r2=sdSerpInv(p+rs2,H,Dmax);
